...
def get_rankings():
    # Vitórias > Menos Derrotas > Saldo > GP
...
def setup_playoffs():
    ...
    seeds = [t for t in get_rankings() if t in qualified]
...
if st.session_state.phase == 'setup':
    ...
    st.title(f"⚽ Rodada {len(st.session_state.rounds)} (Sistema Suíço)")
    curr = st.session_state.rounds[-1]
    
    if curr['bye']: 
        st.warning(f"✨ **Bye desta rodada:** {curr['bye']['name']} (+1 Vitória automática)")
    
    # Verifica se a fase suíça deve acabar
    ativos = [t for t in st.session_state.teams if t['status'] == 'Ativo']
    if not ativos:
        st.success("🏁 Fase Suíça Concluída! Todos os times foram classificados ou eliminados.")
    ...
    with st.form("form_playoff"):
        ...
        if st.form_submit_button("Avançar para Próxima Fase"):
            if len(winners) == 1:
                ...
                # Pareamento Clássico de vencedores
