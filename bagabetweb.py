...
import random
import io
from streamlit_gsheets import GSheetsConnection  # ADICIONADO: Conexão Sheets

# --- CONFIGURAÇÃO DA PÁGINA ---
st.set_page_config(page_title="Gestor de Torneio Suíço", layout="wide")

# --- CONEXÃO GOOGLE SHEETS (NOVO BLOCO) ---
try:
...
if 'tournament_name' not in st.session_state: # ADICIONADO: Nome do Torneio
    st.session_state.tournament_name = ""

# --- FUNÇÕES AUXILIARES ---

# ADICIONADO: Função para Salvar no Sheets
def save_to_sheets():
    if not SHEET_ENABLED or not st.session_state.teams: return
    # Só salva se tiver nome do torneio
    ...
    try:
        ...
        # st.toast(f"💾 Salvo: {st.session_state.tournament_name}") # Comentei para não poluir, descomente se quiser ver
...
def render_sidebar_stats():
    with st.sidebar:
        ...
        # ADICIONADO: Mostra nome do torneio na sidebar
...
def init_playoffs():
    ...
    st.session_state.playoff_asking_penalties = False
    save_to_sheets() # ADICIONADO: Salva ao iniciar Playoffs
...
def advance_playoff_round(results, waiting_teams, losers=None):
    ...
    if last_round_name == "Finais":
        ...
        if champion:
            ...
            save_to_sheets() # ADICIONADO: Salva Campeão
            return
    ...
    save_to_sheets() # ADICIONADO: Salva ao avançar rodada
...
if st.session_state.phase == 'registration':
    ...
    # ADICIONADO: Campo para nome do torneio (Obrigatório para o histórico)
    st.session_state.tournament_name = st.text_input("Nome do Torneio (Obrigatório para salvar histórico)", value=st.session_state.tournament_name)
    ...
    if st.button("Iniciar Torneio", type="primary"):
        if not st.session_state.tournament_name:
            st.error("⚠️ Por favor, dê um nome ao torneio antes de iniciar!")
        else:
            qtd = len(st.session_state.teams)
            if 6 <= qtd <= 16:
                st.session_state.phase = 'swiss'
                generate_swiss_round()
                save_to_sheets() # ADICIONADO: Salva estado inicial
                st.rerun()
            else:
                ...
                if missing_input:
                    ...
                    if not st.session_state.swiss_asking_penalties:
                        ...
                        if has_new_draw:
                            ...
                            save_to_sheets() # ADICIONADO: Salva rodada completa
