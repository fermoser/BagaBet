...
if st.session_state.phase == 'setup':
    ...
    # IMPORTANTE: Esse form agora renderiza TODAS as rodadas ativas no playoffs (Final, 3º lugar, etc)
    with st.form("f_playoff"):
        for idx, p_round in enumerate(st.session_state.playoffs):
            st.subheader(f"📍 {p_round['label']}")
            for i, m in enumerate(p_round['matches']):
                c1, g1c, g2c, c2 = st.columns([2,1,1,2])
                g1 = g1c.number_input(f"{m['home']['name']}", 0, 100, key=f"g1_{idx}_{i}")
                g2 = g2c.number_input(f"{m['away']['name']}", 0, 100, key=f"g2_{idx}_{i}")
                p1, p2 = 0, 0
                if g1 == g2:
                    pc1, pc2 = st.columns(2)
                    p1 = pc1.number_input(f"Pên {m['home']['name']}", 0, 100, key=f"p1_{idx}_{i}")
                    p2 = pc2.number_input(f"Pên {m['away']['name']}", 0, 100, key=f"p2_{idx}_{i}")
                    if p1 == p2: ready = False
                
                # Armazena quem ganhou e quem perdeu nesta rodada específica
        ...
        btn_txt = "Confirmar e Avançar" if not any(r['label'] == "Grande Final" for r in st.session_state.playoffs) else "Finalizar Torneio"
        if st.form_submit_button(btn_txt):
            if not ready:
                st.error("Empates precisam ser resolvidos nos pênaltis!")
            else:
                # Atualiza estatísticas globais
                for item in (venc + derr):
                    t_obj = next(t for t in st.session_state.teams if t['id'] == item['t']['id'])
                    t_obj['goals_for'] += item['g']
                    t_obj['goals_against'] += item['gc']
                    t_obj['goal_diff'] = t_obj['goals_for'] - t_obj['goals_against']
                
                # Checa se a Final acabou de acontecer
