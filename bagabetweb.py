...
if st.session_state.menu_ativo == "Jogos":
        ...
        with st.container(border=True):
            ...
            if sou_admin:
                if not j['finalizado']:
                    ...
                    if st.button("🔄 Reabrir Jogo", key=f"r{i}"):
    ...
    if rank:
        ...
        m1.metric("Líder", df_r.iloc[0]['Apostador'], money_raw(df_r.iloc[0]['Saldo']))
        m2.metric("Total Movimentado", money_raw(df_r['Investido'].sum()))
        m3.metric("Apostadores", len(df_r))
    ...
    # AQUI ESTÁ A CORREÇÃO: Usando dataframe simples sem o gradiente que causou erro
    st.dataframe(df_c, use_container_width=True)

elif st.session_state.menu_ativo == "Admin":
    if sou_admin:
        st.title("⚙️ Painel Admin")
        with st.expander("🔍 Monitor de Dados"):
            c1, c2 = st.columns(2)
            c1.markdown("**Raw Sheets**"); c1.dataframe(st.session_state.df_raw[['a', 'b', 'finalizado', 'apostas_abertas']])
            c2.markdown("**App Processed**"); diag = [{"Jogo": f"{j['a']}x{j['b']}", "Fim": j['finalizado'], "Aberto": j['apostas_abertas']} for j in st.session_state.jogos]
            c2.table(diag)
        st.divider()
        df_ed = pd.DataFrame([{"Time": ""} for _ in range(4)])
        edited = st.data_editor(df_ed, num_rows="dynamic", use_container_width=True)
        if st.button("🚀 CRIAR NOVO TORNEIO", type="primary", use_container_width=True):
