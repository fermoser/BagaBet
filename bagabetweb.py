...
if 'torneio_ativo' not in st.session_state:
    ...
    if menu == "⚙️ Admin":
        if is_admin:
            st.subheader("Configurar Confrontos")
    ...
    # --- ABA JOGOS (RESULTADOS E APOSTAS) ---
    elif menu == "🏟️ Jogos":
        if jogos_df.empty: st.info("Gere os jogos no Admin.")
            ...
            for idx, row in jogos_df.iterrows():
                with st.container(border=True):
                    ...
                    c3.markdown(f"<p style='font-size:20px;'><b>{row['b']}</b></p>", unsafe_allow_html=True)

                    # ÁREA ADMIN: LANÇAR PLACAR
                    if is_admin:
                    ...
                    with st.expander("🤑 Apostas"):
                        ...
                        if not is_done(row.get('finalizado')):
                            with st.form(f"f_ap{idx}"):
                                ...
                                u_nome = ca1.text_input("Seu Nome")
                                u_valor = ca2.number_input("Valor R$", 1, 500, 10)
                                opcs = ["A", "B", "Empate"] if formato == "LIGA" else ["A", "B"]
                                u_palp = ca3.radio("Vencedor:", opcs, horizontal=True)
    ...
    # --- ABA CLASSIFICAÇÃO (LIGA) ---
        ...
        else: st.info("Modo Copa: Acompanhe os resultados agregados na aba Jogos.")

    # --- ABA RANKING FINANCEIRO ---
    elif menu == "🤑 Ranking":
        st.subheader("💰 Lucros dos Apostadores")
        grana = {}
        for _, r in jogos_df.iterrows():
            if is_done(r.get('finalizado')):
                if formato == "LIGA":
        ...
        else: st.info("Aguardando jogos finalizados para calcular o lucro.")
